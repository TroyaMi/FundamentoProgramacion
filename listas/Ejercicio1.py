#Dada una lista, intercambiar los elementos de afuera hacia adentro. Primero con último, 
#segundo con penúltimo, etc
...
#se puede cambiar len(A) por n que es el número de elementos de la lista

for i in range(0, int(len(A)/2)): #len(lista) devuelve el número de elementos de la lista
    T = A[i]
    A[i] = A[len(A)-(i+1)]
    A[len(A)-(i+1)] = T
...
'''
len(miLista) #devuelve el número de elementos de una lista
miLista = []
...
#Funciones anidadas
miLista = ["Pedro", "Juana", 25, "Marcos"]
miLista.remove(miLista[miLista.index("Juana")])
print(miLista[:])
'''